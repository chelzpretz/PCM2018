...
### Simulating Data -- Corralated data
...
#Simulate one tree of 50 tips and two datasets on that tree (x and y), using the simulation methods we used above or other R packages. 


tr <- pbtree(n=50) # a pure birth tree

#Refer to the "Trait simulations" and "Tree simulations" paragraphs on the CRAN phylogenetics task view (https://cran.r-project.org/web/views/Phylogenetics.html). 
#Generate independent contrasts for x and y, then calculate the correlation coefficient (cor.test in R) or least-squares regression coefficient of determination (lm in R) and p-values in R. 


# a correlated pair of characters on a phylogeny:
xy <- sim.char(tr, matrix(c(50,15,15,50), 2, byrow = T))[,,1]
dimnames(xy)[[2]] <- c('x', 'y')
layout(matrix(1:2, 1, 2))
fancyTree(tr, "contmap", x = xy[, 1])
phylomorphospace(tr, xy, label = 'horizontal', cex = 0.5)
...
#Compare your analysis of the independent contrasts against your analysis of the untransformed data.
 ### Reminder of PICs function 
...
## That data less significant with when PIC is ran. There is phylgenecital signal with these traits.

########
# Question 2 
########
# Simulate 1000 trees of 50 tips each and two uncorrelated traits on each tree under a Brownian motion process. 
# For each dataset, calculate the mean, the Pearson product-moment (cor.test), and the p-value for the correlation using both standard methods and independent contrasts. 
# How often should you expect to find a P < 0.05? How often do you for both the PIC and standard methods? and which yields a better estimate of the mean? 
#Is either biased? is there a lower variance to one or the other?

## Create a container or just print screen 
data <- matrix(data = NA, nrow = 1000, ncol = 2)


## Function to pull out p-value from summary
...
for(i in c(1:1000)){
  ...
  pic_fit <- lm(y.ic ~ x.ic + 0)
  data_fit <- lm(y ~ x, as.data.frame(xy))
  
  data[i,1] <- lmp(pic_fit)
  data[i,2] <- lmp(data_fit)
   
  print(i)
}

# ratio of how many times the p-value is more than 0.05

# PICs p-value

pic_expected <- for(i in 1:col(data){
  if(p > 0.05)
}
...
# look for ansteral state for root state